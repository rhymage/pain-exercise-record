...
export async function GET(request:Request){
 ...
 const access=recordAccess();if(!access)return reply(request,{error:'기록 저장소 설정을 확인해 주세요.'},503);
...
export async function PUT(request:Request){
 if(!originAllowed(request))return reply(request,{error:'허용되지 않은 요청이에요.'},403);
 const access=recordAccess();if(!access)return reply(request,{error:'기록 저장소 설정을 확인해 주세요.'},503);
 let day:Day;try{const body=await request.text();if(body.length>30000)return reply(request,{error:'입력 내용이 너무 길어요.'},400);day=JSON.parse(body);if(!validDay(day))return reply(request,{error:'날짜, 통증(0~5), 운동 시간(0~1440분)을 확인해 주세요.'},400);}catch{return reply(request,{error:'입력 형식을 확인해 주세요.'},400);}
